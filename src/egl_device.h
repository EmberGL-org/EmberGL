...
// new
template<class HWDevice> struct graphics_device_traits;
template<class HWDevice> class graphics_device;
template<e_pixel_format pixel_fmt, unsigned width, unsigned height> class graphics_device_mem;
//----------------------------------------------------------------------------
...
public:
  ...
  template<class PSC> EGL_INLINE void dispatch_pso(const PSC&);
  EGL_INLINE void commit();
  template<class TShader> EGL_INLINE void commit(const TShader&);
  //--------------------------------------------------------------------------
  ...
  template<class IPShader> void draw_hline(int16_t x_, int16_t y_, uint16_t width_, const IPShader&);
  template<class IPShader> void draw_vline(int16_t x_, int16_t y_, uint16_t height_, const IPShader&);
  template<class IPShader> void draw_rect(int16_t x_, int16_t y_, uint16_t width_, uint16_t height_, const IPShader&);
  template<class IPShader> void fast_draw_hline(uint16_t x_, uint16_t y_, uint16_t width_, const IPShader&);
  template<class IPShader> void fast_draw_vline(uint16_t x_, uint16_t y_, uint16_t height_, const IPShader&);
  template<class IPShader> void fast_draw_rect(uint16_t x_, uint16_t y_, uint16_t width_, uint16_t height_, const IPShader&);
  //--------------------------------------------------------------------------
...
protected:
  ...
  template<e_pixel_format rt0_fmt> void init_default_tile_shader();
  //--------------------------------------------------------------------------
...
private:
  template<class HWDev, class Cfg> friend struct rasterizer_memory;
...
public:
  // device properties
  enum {fb_width=width, fb_height=height};
